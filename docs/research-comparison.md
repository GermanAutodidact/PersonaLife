...
| [AiMemory](https://github.com/GermanAutodidact/AiMemory) | `src/aimemory/database.py`, `models.py`, `store.py`, `tests/test_store.py`, packaging metadata | Transactional namespace-scoped immutable records, collision handling and context/search | Apache-2.0 | First external memory provider; exercised against fetched actual source. PersonaLife's ledger remains authoritative. |
